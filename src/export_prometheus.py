...
DEFAULT_QUERIES = {
    "container_cpu_usage_seconds_total_rate": 'sum(rate(container_cpu_usage_seconds_total{namespace="sock-shop"}[1m])) by (pod)',
    "container_memory_working_set_bytes": 'sum(container_memory_working_set_bytes{namespace="sock-shop"}) by (pod)',
    "kube_pod_container_status_restarts_total": 'sum(kube_pod_container_status_restarts_total{namespace="sock-shop"}) by (pod)',
    "frontend_requests": 'sum(rate(request_duration_seconds_count{namespace="sock-shop"}[1m]))',
    "frontend_latency_sum": 'sum(rate(request_duration_seconds_sum{namespace="sock-shop"}[1m]))',
}
...
def main() -> None:
    ...
    parser.add_argument("--out", default="data/prometheus_sockshop_metrics.csv")
