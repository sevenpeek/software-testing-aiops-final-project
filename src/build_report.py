...
def main() -> None:
    ...
    r = meta.add_run("学生：李响    学号：2311467")
    r.font.name = "Microsoft YaHei"
    r.font.size = Pt(10.5)
    ...
    add_para(
        doc,
        "论文选择 KDD 2020 的 USAD: UnSupervised Anomaly Detection on Multivariate Time Series。"
        "本实验目标是将 SockShop 的多变量 KPI 监控数据转化为滑动窗口时间序列，训练 USAD 模型识别故障注入后的异常窗口，"
        "并通过异常分数、重构误差和检测指标评估复现效果。"
    )
    ...
    add_para(
        doc,
        "实验设计包括两条闭环：先使用 SockShop 风格示例 KPI 数据完成 USAD 可控复现，再在 Online Boutique 中采集真实 KPI。"
        "真实 KPI 采集阶段使用 kubectl scale 将 productcatalogservice 缩放到 0 个副本制造可重复故障窗口，后续又补充 ChaosMesh PodChaos 实际注入验证。"
    ...
    add_table(
        ...
        ["阶段", "本大作业独立完成内容", "对应文件"],
        [
            ["数据准备", "生成示例 SockShop KPI，并采集 Online Boutique 真实 KPI 与 Prometheus 探针指标", "src/generate_sample_data.py, src/collect_online_boutique_metrics.py"],
    ...
    add_code(doc, "python src/generate_sample_data.py --out data/sample_sockshop_metrics.csv")
    add_code(doc, "python src/run_usad.py --input data/sample_sockshop_metrics.csv --out outputs")
    add_para(
        doc,
    ...
    add_code(
        doc,
        "python src/export_prometheus.py --prometheus http://127.0.0.1:9090 --start 2026-06-01T10:00:00 --end 2026-06-01T11:00:00 --step 30 --out data/prometheus_sockshop_metrics.csv",
    )
    ...
    add_para(
        ...
        "本机浏览器兼容性验证以 Chrome headless 作为代表浏览器完成；未额外覆盖 Edge/Firefox，多浏览器覆盖作为后续改进项。"
    )
    add_table(
        doc,
        ["测试工具", "实际产物", "关键结果"],
    ...
    add_image(doc, ROOT / "outputs_selenium" / "selenium_home.png", "图 5 Selenium 打开 Online Boutique 首页截图", 5.8)
    add_image(doc, ROOT / "docs" / "screenshots" / "jmeter_online_boutique_report.png", "图 6 JMeter HTML 测试报告截图", 5.8)
    ...
    add_image(doc, ROOT / "outputs" / "anomaly_score.png", "图 3 USAD 异常分数曲线", 5.8)
    add_image(doc, ROOT / "outputs" / "reconstruction_error.png", "图 4 各 KPI 平均重构误差", 5.8)
    ...
    add_para(
        ...
        "该阶段采用 kubectl scale 是为了获得边界清晰、便于标注的真实 KPI 数据；ChaosMesh PodChaos 作为后续补充验证，用于证明故障注入工具链已实际打通。"
    )
    add_table(
        doc,
        ["真实数据指标", "结果"],
    ...
    add_image(doc, ROOT / "outputs_online_boutique_real" / "anomaly_score.png", "图 7 Online Boutique 真实 KPI 的 USAD 异常分数", 5.8)
    add_image(doc, ROOT / "outputs_online_boutique_real" / "reconstruction_error.png", "图 8 Online Boutique 真实 KPI 的重构误差", 5.8)
    add_para(
        doc,
    ...
    add_image(
        ...
        "图 9 Online Boutique 的 Grafana 应用性能与运维监控仪表盘",
        6.2,
    )
    ...
    add_para(
        doc,
        "重构误差可以用于解释异常来源。本次实验中，frontend_error_rate、catalogue_memory_usage 和 frontend_p95_latency_ms "
        "位于重构误差排名前列，符合网络延迟与 catalogue Pod 故障对前端体验和服务资源指标产生影响的预期。"
    ...
    add_bullets(
        ...
        [
            ...
            "真实环境应采集更长时间的 Prometheus 数据，并分别记录 kubectl scale 故障窗口与 ChaosMesh 注入起止时间。",
            "当前已完成真实 Online Boutique KPI 检测，并补充 Prometheus/Grafana 仪表盘截图；后续可将异常分数继续写回监控系统形成告警闭环。",
            "可进一步对比 Donut、SR-CNN、OmniAnomaly 等模型，形成多算法对比。",
            "可将异常分数写回 Prometheus 或 Grafana，形成自动告警闭环。",
        ],
    )

    add_heading(doc, "十二、结论")
    add_para(
        doc,
    ...
    out = out_dir / "2311467_李响_软件测试与维护大作业报告.docx"
    doc.save(out)
    print(out)
