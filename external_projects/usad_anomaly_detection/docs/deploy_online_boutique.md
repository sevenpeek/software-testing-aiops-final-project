...
当前 `final_project` 已围绕 Online Boutique 补齐复杂微服务系统部署、测试、监控、故障注入和 USAD 检测证据。本指南记录复现部署与采集过程。
...
建议给 Minikube 分配更多资源。Online Boutique 服务数量较多，资源不足时容易出现 `CrashLoopBackOff` 或镜像拉取/启动超时。
...
如果你已经部署过 Prometheus/Grafana，可以继续复用工具；但建议在大作业中单独记录 Online Boutique 的指标采集过程。
...
本项目的 `src/export_prometheus.py` 默认查询 `online-boutique` namespace，可按实际 Prometheus 指标名称继续扩展查询项。
