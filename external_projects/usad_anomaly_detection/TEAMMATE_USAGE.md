...
3. `report/2311467_李响_2313163_陈祖名_软件测试与维护大作业报告.docx`
   - 已生成的双人版 Word 报告，可继续润色。
...
- 示例复现数据：`data/sample_kpi_metrics.csv`，用于可控验证 USAD 流程。
...
- Selenium：本机已完成基础 Chrome headless 验证，并补充 Edge 浏览器兼容性验证。
...
运行 ChaosMesh 真实 KPI 采集并自动跑 USAD：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/run_chaosmesh_kpi_usad.ps1
```

需要提前保证 Minikube 正常运行、Online Boutique 可通过 `http://127.0.0.1:8080` 访问、ChaosMesh 已安装，并且 `scripts/chaos-online-boutique-productcatalog-podkill.yaml` 可成功执行。
