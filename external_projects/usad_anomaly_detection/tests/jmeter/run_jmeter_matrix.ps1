...
if (-not (Test-Path -LiteralPath $jmeter)) {
  throw "Cannot find $jmeter"
}

$plan = Join-Path $PSScriptRoot "online_boutique_load_test.jmx"
$outDir = Join-Path $root "outputs_jmeter_matrix"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$summaryItems = @()
foreach ($threadCount in $Threads) {
  $caseDir = Join-Path $outDir ("threads_{0}" -f $threadCount)
  $jtl = Join-Path $caseDir "online_boutique_load_test.jtl"
  $html = Join-Path $caseDir "html"
  $tmpPlan = Join-Path $caseDir "online_boutique_load_test.runtime.jmx"
  ...
  $threadGroup.SelectSingleNode("stringProp[@name='ThreadGroup.num_threads']").InnerText = [string]$threadCount
  ...
  $summaryItems += ("{0}={1}" -f $threadCount, $jtl)
}
...
& $python (Join-Path $root "src\summarize_jmeter.py") --out-dir $outDir @summaryItems
